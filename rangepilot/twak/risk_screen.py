...
class TWAKRiskClient:
    ...
    def screen_token(self, token_address: str, chain_id: int = 56) -> RiskReport:
        ...
        level = str(data.get("risk_level", data.get("riskLevel", "unknown"))).lower()
...
class OfflineHeuristicScreen:
    ...
    def screen_token(self, token_address: str, chain_id: int = 56) -> RiskReport:
        ok = isinstance(token_address, str) and token_address.startswith("0x") \
             and len(token_address) == 42
        flags = [] if ok else ["malformed_address"]
        return RiskReport(token_address, self.provider,
                          "unknown" if ok else "high", flags,
